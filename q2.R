...
#e
databp_pmm <- complete(mice(data = databp, method = "pmm", m = 1, donors = 1))

cor(databp_pmm, method = "pearson")

##########

cc_lm <- lm(data = databp_cc, recovtime ~ .)
...
for (i in 1:length(na_ind)){
  while (databp_sri_sr$recovtime[na_ind[i]] < 0){
    databp_sri_sr$recovtime[na_ind[i]] <- predict(cc_lm, databp_sri_sr[na_ind[i], ]) + rnorm(n = 1,
                                                                                          mean = 0,
                                                                                          sd = sigma(cc_lm))
  }
}
...
for (i in 1:length(sq_diff)){
  sq_diff[i] <- list((cc_lm$fitted.values - predict(cc_lm, databp[na_ind[i], ]))^2)
  to_select[i] <- which.min(sq_diff[[i]])
}

for (i in 1:length(na_ind)){
  databp_pmm_sr$recovtime[na_ind[i]] <- databp_cc$recovtime[which.min(sq_diff[[i]])]
}
...
mean(databp_cc$recovtime); mean(databp_mi_sr$recovtime); mean(databp_ri_sr$recovtime); mean(databp_sri_sr$recovtime); mean(databp_pmm_sr$recovtime)
